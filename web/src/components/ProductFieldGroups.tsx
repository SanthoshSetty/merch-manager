...
const AIEnhancedSelect = memo(({
  ...
  const generateContent = async (useCustomInstructions = false) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        console.log('🔍 [AIEnhancedSelect] Field generation - Grounded sources received:', result.grounded_sources);
        console.log('🔍 [AIEnhancedSelect] Field generation - Sources count:', result.grounded_sources?.length || 0);
        console.log('🔍 [AIEnhancedSelect] Field generation - Raw API response:', result);
  ...
  const acceptGeneration = () => {
    console.log('🔍 [AIEnhancedSelect] Accepting generation - Sources before:', groundedSources);
    console.log('🔍 [AIEnhancedSelect] Accepting generation - Sources count:', groundedSources.length);
    console.log('🔍 [AIEnhancedSelect] Accepting generation - lastGeneratedContent:', lastGeneratedContent);
    onFieldChange(fieldName, lastGeneratedContent);
    setShowRefinement(false);
    setHasGenerated(true);
    console.log('🔍 [AIEnhancedSelect] Accepting generation - hasGenerated set to true');
    console.log('🔍 [AIEnhancedSelect] Accepting generation - Final state check - hasGenerated:', true, 'showRefinement:', false, 'sourcesLength:', groundedSources.length);
  };
...
const AIEnhancedSwitch = memo(({
  ...
  const generateContent = async (useCustomInstructions = false) => {
    ...
    try {
      ...
      if (result.success) {
        setLastGeneratedContent(result.content);
        setGroundedSources(result.grounded_sources || []);
        console.log('🔍 [AIEnhancedSwitch] Field generation - Grounded sources received:', result.grounded_sources);
        console.log('🔍 [AIEnhancedSwitch] Field generation - Sources count:', result.grounded_sources?.length || 0);
        console.log('🔍 [AIEnhancedSwitch] Field generation - Raw API response:', result);
        setHasGenerated(true);
  ...
  const acceptGeneration = () => {
    ...
    setShowRefinement(false);
    setHasGenerated(true); // Keep track that this field has been generated
  };
...
const AIEnhancedTextField = memo(({ 
  ...
  useEffect(() => {
    console.log('🔍 [AIEnhancedTextField] State changed:', {
      fieldName,
      hasGenerated,
      showRefinement,
      sourcesCount: groundedSources?.length || 0,
      lastGeneratedContentLength: lastGeneratedContent?.length || 0
    });
  }, [fieldName, hasGenerated, showRefinement, groundedSources, lastGeneratedContent]);
  ...
  const generateContent = async (useCustomInstructions = false) => {
    ...
    try {
      ...
      if (result.success) {
        setLastGeneratedContent(result.content);
        setGroundedSources(result.grounded_sources || []);
        console.log('🔍 [AIEnhancedTextField] Field generation - Grounded sources received:', result.grounded_sources);
        console.log('🔍 [AIEnhancedTextField] Field generation - Sources count:', result.grounded_sources?.length || 0);
        console.log('🔍 [AIEnhancedTextField] Field generation - Raw API response:', result);
        setHasGenerated(true);
        
        // Store in localStorage for debugging
        if (typeof window !== 'undefined') {
          localStorage.setItem(`debug_sources_${fieldName}`, JSON.stringify({
            sources: result.grounded_sources || [],
  ...
  const acceptGeneration = () => {
    console.log('🔍 [AIEnhancedTextField] Accepting generation - Sources before:', groundedSources);
    console.log('🔍 [AIEnhancedTextField] Accepting generation - Sources count:', groundedSources.length);
    console.log('🔍 [AIEnhancedTextField] Accepting generation - lastGeneratedContent:', lastGeneratedContent);
    onFieldChange(fieldName, lastGeneratedContent);
    setShowRefinement(false);
    setHasGenerated(true);
    console.log('🔍 [AIEnhancedTextField] Accepting generation - hasGenerated set to true');
    console.log('🔍 [AIEnhancedTextField] Accepting generation - Final state check - hasGenerated:', true, 'showRefinement:', false, 'sourcesLength:', groundedSources.length);
  };
    ...
    <Box>
      ...
      {showRefinement && lastGeneratedContent && (
        <Box sx={{ mt: 2, p: 2, border: '1px solid', borderColor: 'primary.main', borderRadius: 1, bgcolor: 'primary.50' }}>
          ...
          {/* Grounded Sources - Enhanced Visibility */}
          {groundedSources && groundedSources.length > 0 && (
            <Box sx={{ 
              ...
              <Typography variant="subtitle2" color="info.dark" gutterBottom sx={{ 
                display: 'flex', 
                alignItems: 'center', 
                gap: 1,
                fontWeight: 'bold',
                ...
                AI Sources Referenced ({groundedSources.length})
              </Typography>
              <Stack spacing={1}>
                {groundedSources.map((source, index) => (
                  <Chip
                    key={index}
                    label={source.title || `Source ${index + 1}`}
                    component={source.url && source.url !== 'N/A' ? Link : 'div'}
                    href={source.url && source.url !== 'N/A' ? source.url : undefined}
                    target="_blank"
                    rel="noopener noreferrer" 
                    clickable={source.url && source.url !== 'N/A'}
                    ...
                    sx={{ 
                      ...
                      '&:hover': source.url && source.url !== 'N/A' ? {
          ...
          {typeof window !== 'undefined' && window.location.hostname === 'localhost' && (
            <Box sx={{ mt: 1, p: 1, bgcolor: 'warning.50', borderRadius: 1, fontSize: '0.7rem' }}>
              <Typography variant="caption" color="text.secondary">
                Debug: Country={country}, Sources={groundedSources?.length || 0}, HasGenerated={hasGenerated}, ShowRefinement={showRefinement}
              </Typography>
              {groundedSources && groundedSources.length > 0 && (
                <Box sx={{ mt: 1 }}>
                  ...
                  {groundedSources.map((source, idx) => (
                    <Typography key={idx} variant="caption" color="text.secondary" sx={{ display: 'block', fontSize: '0.6rem' }}>
                      {idx + 1}. {source.title || 'No title'} - {source.url || source.uri || 'No URL'}
                    </Typography>
                  ))}
                </Box>
              )}
      ...
      {/* Sources indicator when content is applied - Enhanced visibility for debugging */}
      {(() => {
        console.log('🔍 [AIEnhancedTextField] Sources indicator render check:', { hasGenerated, showRefinement, sourcesCount: groundedSources?.length, fieldName });
        return null;
      })()}
      
      {/* Debugging section - Always show if sources exist */}
      {typeof window !== 'undefined' && window.location.hostname === 'localhost' && groundedSources && groundedSources.length > 0 && (
        <Box sx={{ mt: 1, p: 2, bgcolor: 'info.50', borderRadius: 1, border: '1px solid', borderColor: 'info.main' }}>
          <Typography variant="caption" color="info.dark" sx={{ fontWeight: 'bold' }}>
            🔍 DEBUG: Grounded Sources Available ({groundedSources.length})
          </Typography>
          <Typography variant="caption" color="text.secondary" sx={{ display: 'block', mt: 0.5 }}>
            hasGenerated: {hasGenerated ? 'true' : 'false'} | showRefinement: {showRefinement ? 'true' : 'false'}
          </Typography>
        </Box>
      )}
      ...
      {typeof window !== 'undefined' && window.location.hostname === 'localhost' && (() => {
        const stored = localStorage.getItem(`debug_sources_${fieldName}`);
        if (stored) {
          const data = JSON.parse(stored);
          return (
            <Box sx={{ mt: 1, p: 1, bgcolor: 'warning.50', borderRadius: 1, fontSize: '0.7rem' }}>
              <Typography variant="caption" color="text.secondary">
                📦 LocalStorage: {data.sources?.length || 0} sources stored
              </Typography>
            </Box>
          );
        }
      ...
      {/* FORCE SHOW SOURCES FOR DEBUGGING - Remove after debugging */}
      {typeof window !== 'undefined' && window.location.hostname === 'localhost' && groundedSources && groundedSources.length > 0 && (
        <Box sx={{ mt: 1, p: 1, bgcolor: 'success.50', borderRadius: 1, border: '1px solid', borderColor: 'success.main' }}>
          <Typography variant="caption" color="success.dark" sx={{ fontWeight: 'bold', display: 'block' }}>
            🚀 FORCE SHOW: {groundedSources.length} Grounded Sources
          </Typography>
          <Stack direction="row" spacing={1} flexWrap="wrap" sx={{ mt: 1 }}>
            {groundedSources.map((source, index) => (
              <Chip
                key={index}
                label={source.title || `Source ${index + 1}`}
                component={source.url && source.url !== 'N/A' ? 'a' : 'div'}
                href={source.url && source.url !== 'N/A' ? source.url : undefined}
                target="_blank"
                rel="noopener noreferrer"
                clickable={source.url && source.url !== 'N/A'}
...
function ProductFieldGroups({ productData, onFieldChange, productId }: ProductFieldGroupsProps) {
  ...
  // Enhanced helper function to detect if a retailer is an official website
  const isOfficialWebsite = (retailerName: string, url: string, brand: string) => {
    const result = classifyOfficialWebsite(retailerName, url, brand);
    
    // Log detailed classification for debugging
    console.log('🔍 Enhanced Official Website Classification (ProductFieldGroups):', {
  ...
  const generateComprehensiveAnalysis = async () => {
    ...
    try {
      ...
      if (result.success && result.data) {
        ...
        console.log('Grounded Sources in response:', result.grounded_sources);
        console.log('Sources array length:', result.grounded_sources?.length || 0);
        
        setComprehensiveAnalysisData(result.data);
        setGroundedSources(result.grounded_sources || []);
        console.log('Comprehensive Analysis Sources set to state:', result.grounded_sources);
        console.log('🚀 State after setting - groundedSources should now be:', result.grounded_sources || []);
        ...
        <Accordion defaultExpanded>
          <AccordionSummary expandIcon={<ExpandMoreIcon />}>
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
              ...
              <Chip label="Generate All Fields" size="small" color="secondary" variant="outlined" />
            </Box>
          </AccordionSummary>
          <AccordionDetails>
            <Stack spacing={3}>
              ...
              <Alert severity="info" sx={{ mt: 1 }}>
                <Typography variant="body2">
                  💡 <strong>Tip:</strong> Use real product names (e.g., "iPhone 15 Pro", "Nike Air Max") for the AI to find and display relevant sources.
                </Typography>
              </Alert>

              {/* Debug info for comprehensive analysis */}
              {typeof window !== 'undefined' && window.location.hostname === 'localhost' && (
                <Box sx={{ mt: 1, p: 1, bgcolor: 'info.50', borderRadius: 1, fontSize: '0.7rem' }}>
                  <Typography variant="caption" color="text.secondary">
                    Comprehensive Analysis Debug: Country={aiMarketCountry}, Sources={groundedSources?.length || 0}, Analysis Complete={!!comprehensiveAnalysisData}
                  </Typography>
                </Box>
              )}

              {/* Comprehensive Analysis Sources Display */}
              {console.log('🚀 Rendering check - groundedSources:', groundedSources)}
              {console.log('🚀 Rendering check - groundedSources.length:', groundedSources?.length)}
              {console.log('🚀 Rendering check - condition result:', groundedSources && groundedSources.length > 0)}
              {groundedSources && groundedSources.length > 0 && (
                <Box sx={{ 
                  mt: 2, 
                  p: 2, 
                  ...
                  <Typography variant="subtitle2" color="success.dark" gutterBottom sx={{ 
                    display: 'flex', 
                    alignItems: 'center', 
                    gap: 1,
                    fontWeight: 'bold',
                    ...
                    <PublicIcon sx={{ fontSize: 18 }} />
                    Comprehensive Analysis Sources ({groundedSources.length})
                  </Typography>
                  <Stack spacing={1}>
                    {groundedSources.map((source, index) => (
                      <Chip
                  ...
                  <Box sx={{ mt: 2 }}>
                    ...
                    <Box sx={{ 
                      ...
                      {groundedSources.map((source, index) => (
                        <div key={index}>
                          {index + 1}. {source.url || source.uri || 'N/A'}
                        </div>
                      ))}
                    </Box>
                  </Box>
